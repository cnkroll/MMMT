...
#
# These function perform the MMMT and FDR test in:
...
Plot_Func<-function(pre,post,mood_modified,p,label,logy,ymin,ymax,ex,FDR_In,FDR_flag,pdfname){
  ...
  legend("bottomleft", inset=c(0,-0.3), xpd=TRUE , c(paste("Median ",label[4],sep=""),paste("Median ",label[5],sep=""), paste("Individual Annual ",label[4]," (",dim(pre)[2]," Years)",sep=""), paste("Individual Annual ",label[5]," (",dim(post)[2]," Years)",sep="")), lty = c(NA,NA,1,1), lwd = c(1.2,1.2,1.2,2,2), pch = c(0,0,NA,NA) , col = c("blue", "black", mycol_lightblue, "snow2" ),cex=0.8,bty = "n")
  
  legend("bottomleft", inset=c(0.7,-0.28), xpd=TRUE , c("               p-value <= 0.01", "    0.01 < p-value <= 0.05", "    0.05 < p-value <= 0.1","               p-value > 0.1"), bty="n", col = c("red", "orange", "yellow","blue"), pch = c(15,15,15,15), y.intersp = 0.8, x.intersp = 0.6,cex=0.8)
  legend("bottomleft", inset=c(0.717,-0.28), xpd=TRUE , "" , bty="n", col = "black", pch = 15, y.intersp = 0.8, x.intersp = 0.6,cex=0.8)
  
  # Add FDR results if FDR = TRUE
