...
# 4)	Plot_Func = a routine to plot MMMT output across a range of percentiles.
...
# 
# The function Plot_func makes a pdf of a plot of the output from the MMMT function. The pre and post matrices are used to plot the percentiles for each year (light lines on the plot) and calculate the medians at each percentile (darker symbols), while the p-values from the MMMT are used to color code the medians for each percentile (red = p-value < 0.01; orange = 0.01 < p-value < 0.05; yellow = 0.05 < p-value < 0.10, and blue = p-value > 0.10).
# 
# References:
# 
# Fligner, M.A., Rust, S.W. 1982. A modification of Mood’s median test for the generalized Behrens-Fisher problem. Biometrika, 69, 221–226.
# 
# Harrell, F.E., and Davis, C.E. 1982. A new distribution-free quantile estimator. Biometrika, 69, 635-640.
# 
# Mofawy, M.H.M.A., Kroll, C.N., and Vogel, R.M. 2022. A graphical FDC-based non-parametric hydrologic alteration test. Journal of Hydrology, submitted February 2022. Draft copy available at www.esf.edu/ere/kroll/publications.
